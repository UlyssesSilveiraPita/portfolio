  ...
  $.fn.ajaxJSSwitch = function (o) {
    ...
    function init() {
      ...
      if ($(customObject.contentHolder).contents().length == 0) {
        ...
        $(customObject.contentHolder)
          .contents()
          .wrapAll("<div class='content'></div>");
      }
      $(customObject.contentHolder).after(
        $(customObject.contentHolder)
          .clone()
          .html("<div class='content'></div>"),
      );
      $(customObject.webSite)
        .css({
          position: "absolute",
          width: "100%",
          height: "100%",
      ...
      $("head").append(
        "<style>#wrapper section {overflow:visible!important}</style>",
      );
      minHeight = customObject.bodyMinHeight;
      //call user function
      customObject.menuInit(
        $(customObject.classMenu),
        $(customObject.classSubMenu),
      );
      ...
      if (location.protocol != "file:") {
        if (location.search.replace("/?", "/")) {
          ...
          initLoad = true;
          var ajaxUrl = requestUrl.href;

if (ajaxUrl.indexOf("?") !== -1) {
  var basePart = ajaxUrl.split("?")[0];
  var routePart = ajaxUrl.split("?")[1].split("&")[0].split("#")[0];

  if (routePart.indexOf(".html") !== -1) {
    ajaxUrl =
      basePart.slice(0, basePart.lastIndexOf("/") + 1) + routePart;
  } else {
...
loadContent(ajaxUrl, true);
      ...
      $("li", customObject.classMenu)
        .hover(menuButtonOver, menuButtonOut)
        .mousedown(menuButtonOver)
        .trigger("mouseout");
      $("*")
        .not("a, li, ul")
    ...
    function followLink() {
      if (
        ...
        !$(this).hasClass(customObject.noLoadAjax)
      ) {
        var linkHref,
          hrefDomenSlice,
          hrefDomen,
        ...
        if (linkHref.indexOf("#") == -1) {
          ...
          followLinkTest =
            linkHrefLower.indexOf(".html") != -1 ||
            linkHrefLower.indexOf(".htm") != -1 ||
            linkHrefLower.indexOf(".php") != -1;
          splash = linkHref != window.location.href + customObject.mainPage;
          if ($(this).parents("form").length == 0) {
            ((hrefDomenSlice = linkHref.slice(
              linkHref.indexOf("://") + 3,
              linkHref.length,
            )),
              (hrefDomen = hrefDomenSlice.slice(
                0,
                hrefDomenSlice.indexOf("/"),
              )));
            if (
              webSiteHref.indexOf(hrefDomen) != -1 &&
              followLinkTest &&
              linkHref.indexOf("assets") == -1
            ) {
              if (
                linkHref != currHref &&
                linkHref.indexOf($(">a", currButton).attr("href")) == -1 &&
                animationComplete &&
                splash
              ) {
                currHref = linkHref;
    ...
    function changeActiveButton() {
      if (
        $("option[value*='" + $(">a", currButton).attr("href") + "']").length ==
        0
      ) {
        $("option", ".select-menu").eq(0).attr({ selected: true });
      } else {
        $("option[value*='" + $(">a", currButton).attr("href") + "']").attr({
          selected: true,
        });
      }
      if (
        $(">a", prevButton).parents("li", customObject.classMenu).length != 0
      ) {
      ...
      currButton = currButton.not(
        currButton.find("." + customObject.noLoadAjax).parent(),
      );
      if (currButton && currButton != "") {
        currButton = $("ul").find(currButton);
        if (
          $(">a", currButton).parents("li", customObject.classMenu).length != 0
        ) {
    ...
    function loadContent(url, push) {
      ...
      xhr = $.ajax({
        url: url,
        beforeSend: function () {
          var prevHolder = $(customObject.contentHolder).not(
            $(customObject.contentHolder).eq(currentPage),
          );
        ...
        success: function (data, textStatus, xhr) {
          ...
          setTimeout(function () {
            currentHolder
              .find(">.content")
              .bind("resize.rainbows", wrapperResize)
              .trigger("resize.rainbows");
          }, 100);
          customObject.pageLoadComplete();
          ...
          if (
            webSiteHref == url ||
            url.lastIndexOf(customObject.mainPage) != -1
          ) {
    ...
    function prevPageAnonateComplete() {
      animationComplete = true;
      $(customObject.contentHolder)
        .not($(customObject.contentHolder).eq(currentPage))
    ...
    function wrapperResize() {
      var margins = customObject.topMargin + customObject.bottomMargin,
        bodyChange = $("body"),
        contentHeight = $(customObject.contentHolder)
          .eq(currentPage)
          .outerHeight(),
        addContentHeight =
          $(customObject.contentHolder)
            ...
            .outerHeight() +
          parseInt(
            $(customObject.contentHolder).eq(currentPage).css("padding-top"),
          ) +
          parseInt(
            $(customObject.contentHolder).eq(currentPage).css("padding-bottom"),
          ),
      ...
      if ($(document).outerWidth() < customObject.deviceSize) {
        margins =
          customObject.topMarginMobileDevices +
          customObject.bottomMarginMobileDevices;
        ...
        if (
          currentUrl.indexOf("index-1-en") !== -1 ||
          currentUrl.indexOf("index-2-en") !== -1
        ) {
          minHeight = 1300;
        }
      }
      if (noFullContent && customObject.fullHeight) {
        ...
        $(customObject.contentHolder)
          ...
          .animate(
            {
              height: $(customObject.contentHolder)
                .eq(currentPage)
                .find(">.content")
                .outerHeight(),
            },
      ...
      contentHeight =
        $(customObject.contentHolder).eq(currentPage).outerHeight() + margins;
      if (contentHeight > minHeight) {
        if ($(document).outerWidth(true) < customObject.deviceSize) {
          ...
          bodyChange
            .stop(true)
            .animate({ "min-height": contentHeight }, 700, "easeInOutCubic");
        }
      } else {
        bodyChange
          .stop(true)
          .animate({ "min-height": minHeight }, 700, "easeInOutCubic");
      }
    }
